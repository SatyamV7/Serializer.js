class Serializer { // Serializer.js v0.0.1 <5th September, 2025> - Author: Satyam Verma <github.com/SatyamV7> - License: LGPL-3.0-or-later
        ...
        this.#serializeComplex = function (data, visited, CallFrame, InvokeReplacer) {
            ...
            if (CallFrame > depth) return `[${data?.constructor?.name || 'Object'}]`;
                ...
                if (data === null) {
                    ...
                    return `DataView {${indentString}byteLength: ${data.byteLength},${indentString}byteOffset: ${data.byteOffset},${indentString}buffer: ${buffer}${smallIndentString}}`;
                    ...
                    return `ArrayBuffer {${indentString}[Uint8Contents]: <${buffer}>,${indentString}byteLength: ${data.byteLength}${smallIndentString}}`;
                    ...
                    return `SharedArrayBuffer {${indentString}[Uint8Contents]: <${buffer}>,${indentString}byteLength: ${data.byteLength}${smallIndentString}}`;
                } else if (data instanceof Error) {
                    return `Uncaught ${data.constructor.name}: ${data.message}\n\nStack Trace: ${data.stack}`;
                } else if (data instanceof WeakRef) {
                    return `WeakRef { ${data.deref() ? this.#serialize(data.deref()) : '<cleared>'} }`;
    ...
    get version() {
        return 'Serializer.js v0.0.1-r0';
    }
